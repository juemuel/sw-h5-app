  ...
  <div class="padding-20">
    ...
    <div class="btn-wrapper mt-10">
      ...
      <el-button type="primary" @click="muteAudio" :disabled="!joined">
        {{ audioTrackMuted ? "Unmute Audio With SetEnabled" : "Mute Audio With SetEnabled" }}</el-button>
      <el-button type="primary" @click="muteVideo" :disabled="!joined">
        {{ videoTrackMuted ? "Unmute Video With SetEnabled" : "Mute Video With SetEnabled" }}</el-button>
    </div>
...
const muteAudio = async () => {
    await audioTrack.value?.setEnabled(audioTrackMuted.value);
    audioTrackMuted.value = !audioTrackMuted.value
  }


  const muteVideo = async () => {
    await videoTrack.value?.setEnabled(videoTrackMuted.value);
