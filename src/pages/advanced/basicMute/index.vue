<template>
  <div class="padding-20">
    ...
    <div class="btn-wrapper mt-10">
      ...
      <el-button type="primary" @click="muteAudio" :disabled="!joined">{{ audioTrackMuted ? "Unmute Audio" :
        "MuteAudio" }}</el-button>
      <el-button type="primary" @click="muteVideo" :disabled="!joined">
        {{ videoTrackMuted ? "Unmute Video" : "Mute Video" }}</el-button>
    </div>
...
const muteAudio = async () => {
  await audioTrack.value?.setMuted(!audioTrackMuted.value);
  audioTrackMuted.value = !audioTrackMuted.value;
}


const muteVideo = async () => {
  await videoTrack.value.setMuted(!videoTrackMuted.value);
